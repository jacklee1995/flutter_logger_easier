...
abstract class AbstractLogFormatter<T> {
  /// 格式化日志记录
  ///
  /// [record] 是要格式化的日志记录
  /// 返回格式化后的日志字符串
  ...
  factory AbstractLogFormatter.fromFormatString(String formatString) {
    // 这里应该根据格式字符串创建适当的格式化器
    // 实际实现可能会更复杂，可能需要解析格式字符串并创建相应的格式化器
    throw UnimplementedError(
        'This factory method should be implemented in a concrete class');
  }
}

/// 简单的日志格式化器实现
class SimpleLogFormatter implements AbstractLogFormatter {
  @override
  String format(LogRecord record) {
  ...
  @override
  AbstractLogFormatter clone() => SimpleLogFormatter();
